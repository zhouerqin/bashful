...
将 `logger.sh`（和按需的 `interactive.sh`）放入您的项目目录，并在脚本中引入：
...
日志库严格只处理**"已经发生或正在发生的单向事实"**。交互输入（`Y/N` 确认、表单填写等）属于"双向阻塞 I/O"，独立封装在 `interactive.sh` 中。两个库可以协同使用，也可以只取其一——例如 CI/CD 环境只需要日志，可以只 source `logger.sh`。
...
| 函数 | 说明 |
|------|------|
| `ui_prompt <prompt> [default]` | 显示提示并读取用户输入，支持默认值 |
| `ui_prompt_secret <prompt>` | 隐藏输入（密码/密钥），返回值捕获到变量 |
| `ui_confirm <prompt> [y/n]` | Y/N 确认提示，返回 0 或 1；非 TTY 时自动使用默认值 |
...
- **信号管理**：本库不注册任何 `trap` 处理器。信号的捕获与清理由调用方负责。
- **临时文件**：本库不创建任何临时文件。`run_cmd` 的输出仅存在于内存变量中。
- **颜色降级**：颜色输出基于终端检测 (`[[ -t 1 || -t 2 ]]`) 和 [`NO_COLOR`](https://no-color.org/) 标准。在管道/重定向场景中颜色自动关闭。如需强制关闭，设置 `NO_COLOR=1`。
- **非 TTY 行为**：`interactive.sh` 中的函数在检测到 STDIN 非终端时自动静默降级：`ui_prompt` 返回默认值，`ui_prompt_secret` 返回空串，`ui_confirm` 使用预设默认值。无需特殊适配即可在 CI/CD 或 Cron 中运行。
